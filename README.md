...
- 本地所有 `xcodebuild` 参数探测与验证需求（含 `-list` / `-showdestinations` / build/test）必须在非沙盒项目环境通过 wrapper 执行：由主 Agent 使用 `functions.exec_command` 并设置 `sandbox_permissions="require_escalated"` 启动目标项目根目录的 `codex_verify.sh`，若项目未接入则回退到本机 `~/.codex/bin/codex_verify`。不得直接调用 `xcodebuild` 二进制，也不要让多个 Agent 各自裸跑 `xcodebuild`；wrapper 会自动接入 shared build-queue daemon，把验证型 `xcodebuild` 串行排队执行，并统一使用 Xcode 系统 DerivedData。
