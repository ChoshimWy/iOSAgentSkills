...
Use and maintain GitNexus PDG query behavior for anchored control/data-dependence reads over `--pdg` indexes.
...
```json
{
  "goal": "anchored target, mode controls/flows, optional variable, repo name, PDG availability.",
...
```json
{
  ...
  "pdg_findings": [],
  "known_risks": [],
  "next_action": "read-source | taint-analysis | ask-user | blocked"
}
```
## Exit Conditions
...
Use for `pdg_query` semantics; use `gitnexus-taint-analysis` for source-to-sink taint findings and PDG internals.
## Token Budget
