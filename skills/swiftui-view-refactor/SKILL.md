...
description: 以小而明确的子视图、MV 优先的数据流、稳定视图树、显式依赖注入和正确的 Observation 用法为默认策略，重构已有 SwiftUI 视图文件；不用于新页面模式设计，也不处理通用非 SwiftUI 重构。
---
...
- 如果任务只是把既定页面模式落地成普通 SwiftUI 代码，切换到 `swiftui-feature-implementation`。
