...
## 强制收尾验证
- 如果当前任务没有进入 `codex-subagent-orchestration`，或当前轮只能以单 Agent 执行，本 skill 完成实现后也不要直接跳到最终门禁；默认后续链路按固定四步执行：`uikit-feature-implementation -> testing -> code-review -> verify-ios-build`。
