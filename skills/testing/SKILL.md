...
- 同机同仓如果有多个 Codex / Claude CLI 并发处理同一 Xcode 项目，测试阶段的项目环境 `xcodebuild` 也应统一复用验证 wrapper：优先目标项目根目录的 `codex_verify.sh`，若项目未接入则回退到本机 `~/.codex/bin/codex_verify`。wrapper 默认使用 `XCODE_DERIVED_DATA_MODE=isolated-preferred`，给每个 CLI 分配专属 DerivedData slot，并在需要时回退到 `system-serial`。
- 默认验证缓存策略不是共享同一个系统 DerivedData 根目录，而是由 wrapper 为当前 CLI 注入专属 `-derivedDataPath`；首次 seed 策略默认 `XCODE_DERIVED_DATA_SEED_MODE=once`，需要强制重建时再用 `XCODE_DERIVED_DATA_REFRESH=1` 或 `XCODE_DERIVED_DATA_SEED_MODE=always`。
