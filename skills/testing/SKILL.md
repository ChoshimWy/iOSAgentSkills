...
- 同机同仓如果有多个 Codex / Claude CLI 并发处理同一 Xcode 项目，测试阶段的项目环境 `xcodebuild` 也应统一复用验证 wrapper：优先目标项目根目录的 `codex_verify.sh`，若项目未接入则回退到本机 `~/.codex/bin/codex_verify`。wrapper 会自动接入 shared build-queue daemon，把验证型 `xcodebuild` 串行排队执行，并统一使用 Xcode 系统 DerivedData。
- 默认验证缓存策略不再暴露 CLI 专属 slot/seed 公开配置；wrapper 会把验证型 `xcodebuild` 统一提交给 daemon，并固定使用 Xcode 系统 DerivedData。若用户仍设置旧 `XCODE_DERIVED_DATA_*` / `CODEX_DERIVED_DATA_SLOT`，wrapper 会 fail-fast。
