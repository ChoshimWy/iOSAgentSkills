...
- 非编排 / 单 Agent 的实现型任务进入固定四步链路时，作为 `code-review` 之后、`verify-ios-build` 之前的第三步测试阶段。
...
- 如果当前改动不适合新增测试代码，也必须在本阶段明确给出 `no_test_reason` 与已覆盖的验证依据，然后再进入 `verify-ios-build`。
...
## 与其他技能的关系
- 如果当前任务属于非编排 / 单 Agent 的实现链路，本 skill 默认承接 `code-review` 后的第三步；无论是否新增测试代码，都要给出测试结论或 `no_test_reason`，然后再进入 `verify-ios-build`。
