...
- 如果需要补跑项目环境验证，应统一复用验证 wrapper：优先目标项目已接入的 repo-tracked `codex_verify.sh`，若项目未接入则回退到本机 `~/.codex/bin/codex_verify`。wrapper 默认使用 `XCODE_DERIVED_DATA_MODE=isolated-preferred` 为每个 CLI 分配专属 DerivedData slot；只有显式 `system-serial` 或隔离模式命中锁冲突时，才回退到串行系统缓存路径。
...
- `codex_verify.sh` / `~/.codex/bin/codex_verify` 是验证入口层的缓存隔离与并发控制机制；它们不替代本门禁的证据裁决，只负责为真正需要执行的项目环境验证分配 CLI 专属 DerivedData，并在必要时回退串行系统缓存。
