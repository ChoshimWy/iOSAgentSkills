...
| --- | --- | --- | --- | --- |
| `ios-base` | 默认 iOS/Swift 实现技能 | 通用 iOS/Swift 业务开发、常规组件实现、普通架构落地 | 深度并发与类型抽象、SwiftUI 专项模式、构建配置、官方文档检索 | `swift-expert`、`swiftui-ui-patterns`、`xcode-build`、`apple-docs` |
...
| Skill | 角色 | 主触发场景 | 不要触发的场景 | 切换到 |
| --- | --- | --- | --- | --- |
| `swift-expert` | 进阶 Swift 设计 | `actor`、`Sendable`、`PAT`、类型擦除、指标驱动性能/内存、多平台可用性 | 普通 iOS 开发、常规 SwiftUI 页面实现 | `ios-base`、`swiftui-ui-patterns` |
| `swiftui-ui-patterns` | 新建 SwiftUI 页面与模式选型 | 新页面、组件模式、`TabView`、导航、布局结构 | 既有大 view 清理、视觉设计系统、运行时性能专项 | `swiftui-view-refactor`、`ui-ux-pro-max`、`swiftui-performance-audit` |
...
| `testing` | 测试编写专项 | 单元测试、UI 测试、Mock/Stub/Spy、异步测试 | 一次性 `xcodebuild` 校验、代码审查、运行时排障 | `verify-ios-build`、`code-review`、`debugging` |
...
| `debugging` | 运行时排障 | crash、泄漏、卡顿、启动慢、异常 | 纯静态审查、构建配置设计 | `code-review`、`swiftui-performance-audit`、`xcode-build` |
| `swiftui-performance-audit` | SwiftUI 运行时性能诊断 | 更新过多、滚动卡顿、CPU/内存过高、布局抖动 | 新页面设计、普通视图重构、泛化 crash 排查 | `swiftui-ui-patterns`、`swiftui-view-refactor`、`debugging` |
