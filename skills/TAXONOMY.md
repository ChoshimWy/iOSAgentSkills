...
- 默认优先切到 `codex-subagent-orchestration` 做自适应编排：先按 `lite` / `standard` / `full` 选择角色，再协调编码、调试、性能、审查、测试与最终证据门禁；构建、测试、自动化、截图与日志优先切 `ios-automation`、`xcode-build`、`final-evidence-gate` 或 `verify-ios-build`。
