...
- 如果同一任务后续还要执行 `verify-ios-build`，最终门禁默认复用这次模拟器定向测试的 workspace / scheme / destination 基线；不要无说明切到另一个 scheme。
...
- 如果同一任务里已经先跑过模拟器定向测试，`verify-ios-build` 默认复用同一套 workspace / scheme / destination 基线；不要无说明切到不同 scheme。
