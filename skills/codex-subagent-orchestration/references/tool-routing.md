...
- 用户明确指定 `.sketch` 源文件、画板或 Sketch 设计真源：启动只读 `design_researcher`，由其专属 `sketchMCP`（`http://localhost:31126/mcp`）读取图层与样式事实并输出 Design Evidence / Design-to-Code source packet；再由 `design-context-compiler` 生成 Canonical UI IR、iOS bindings 与任务级 Agent Packet。MCP 未启动或源文件不可读时报告 blocked，不以截图或既有 UI 猜测替代。
