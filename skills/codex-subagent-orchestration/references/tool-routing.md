...
- 默认收口为定向验证 + 独立 reviewer subAgent `code-review`；`ios-verification` 按需升级，不下放给 subAgent；需要在目标项目环境执行可选验证或越过 sandbox 时，由主 Agent 使用 `functions.exec_command` 并按需设置 `sandbox_permissions=\"require_escalated\"`。
