...
- 责任人：main agent
- 进入条件：任务已完成最小只读定位，具备制定计划所需的上下文
- 通过标准：明确目标、边界、成功标准、`lite` / `standard` / `full` 档位、验证基线，并在任何写文件 / patch 前输出或维护简短实施计划
- 失败处理：补齐缺失信息后重评，不进入实现
- 运行时约束：CP0 不依赖手动 Plan 模式；非 Plan 模式下也必须先完成最小计划，禁止从代码查找直接跳到实现
...
- 责任人：main agent
- 进入条件：CP2 通过，修复轮次收敛
- 通过标准：定向验证或必要验证通过，且独立 reviewer subAgent 执行的 `code-review` 无 `阻塞问题`；reviewer subAgent 不可用时只能 blocked / pending review；`ios-verification` 仅在用户显式要求或高风险时按需补强
