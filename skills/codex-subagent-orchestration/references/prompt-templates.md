...
coder / tester 不再以用户显式要求或 prompt 授权作为硬门槛；当运行时工具可用、写集安全且拆分能提升质量/效率时，主 Agent 可自主调用原生 subAgent。实现链路的 `code-review` 必须调用独立 reviewer subAgent，且 reviewer 默认在 `spawn_agent` 参数里指定 `model="gpt-5.3-codex-spark"`。只有用户明确要求模型分工、任务风险需要或预算/吞吐目标明确时，主 Agent 才为 coder / tester 在 `spawn_agent` 参数里按角色指定 `model` / `reasoning_effort`：
...
- 默认进入 `codex-subagent-orchestration` 不等于必须 spawn 全部 coder / tester subAgent；主 Agent 可按 `lite` / `standard` / `full`，在运行时允许、写集安全且拆分有收益时自主启动最少必要的 coder / tester；实现链路 reviewer subAgent 始终独立启动。
