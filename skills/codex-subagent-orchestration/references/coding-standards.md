...
- 新建 `.swift` / `.h` / `.m` / `.mm` 时先检查同目录现有文件头；若项目使用文件头，必须用 `whoami` 或 `id -un` 的真实结果填写 `Created by`，日期为 `YYYY/M/D`，禁止写 `Codex`、字面量 `$(whoami)` 或占位符；完成前重新打开新增源文件复查。
...
- 新增 `.swift` / `.h` / `.m` / `.mm` 的文件头不符合项目要求时默认阻塞，尤其是 `Created by` 出现 `Codex`、字面量 `$(whoami)`、占位符或非本机用户名。
