...
## main agent
- 默认先选择 `lite` / `standard` / `full` 自适应编排档位；默认进入编排入口不等于必须 spawn 全部 coder / tester subAgent。主 Agent 可在运行时工具可用、工具策略允许、写集安全且拆分有质量/效率收益时自主启动最少必要的 coder / tester subAgent；工具不可用、策略禁止、收益不足或写集不适合并行时，必须显式说明 coder / tester 本轮按单 Agent 执行。实现链路 reviewer subAgent 必须独立启动；不可用时报告 blocked / pending review。
