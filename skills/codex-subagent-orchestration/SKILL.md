...
description: 默认优先使用的 iOS 主 Skill 入口；它先按任务复杂度选择 lite / standard / full 档位，再协调 coder / reviewer / tester / main agent 分工，并在内部路由到实现、调试、性能、测试与最终证据门禁模块；若当前运行时或上层策略要求显式授权 subAgent，而用户尚未授权，则临时回退单 Agent。
---
