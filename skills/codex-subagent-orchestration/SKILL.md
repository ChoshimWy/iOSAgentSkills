...
description: 默认 iOS 主入口：按复杂度选择 lite/standard/full，协调 explorer/builder/reviewer/tester/reporter，并路由到实现、调试、性能、测试和 final-evidence-gate；未授权 subAgent 时回退单 Agent。
---
...
## 角色定位
- 编排型 skill。
- 负责在 Codex 原生能力范围内，先按任务复杂度选择 `lite` / `standard` / `full` 档位，再把实现、调试、性能、审查、测试与最终证据门禁拆成合适的角色协同。
- 不替代具体实现 skill，也不替代 `final-evidence-gate` 的最终裁决；`verify-ios-build` 仅作为升级执行器。
...
- 如果当前运行时、上层策略或用户约束要求显式授权 `subAgent`，而用户尚未授权，则本 skill 只能临时回退单 Agent 编排说明；一旦授权条件满足，应恢复按档位自适应编排。

## 自适应编排档位
- `lite`：极小、单文件、无明确测试面或纯文档/规则小改。优先单 Agent；实现型任务仍必须保留 `code-review` 审查阶段（可由主 Agent 轻量审查，或按需补 `reviewer explorer`）；涉及 Apple Xcode 项目改动时仍必须由主 Agent 执行 `final-evidence-gate`；证据不足或高风险时再升级 `verify-ios-build`。
...
- 任务涉及 Apple Xcode 项目改动时，最终仍由主 Agent 执行 `final-evidence-gate`；证据不足或高风险时再升级 `verify-ios-build`。

## 默认编排
1. 主 Agent 先本地确定目标文件范围、成功标准、编排档位，以及需要复用的 workspace / scheme / destination 基线（如适用）。
   - 若目标工程使用 CocoaPods 且任务涉及私有组件/依赖联调，先额外锁定依赖来源：检查 `Podfile` / `Podfile.lock` / `Pods/Manifest.lock`，确认是否为本地 `:path` Pod；命中时把真实组件仓设为 ownership，把 `Pods/` 副本设为禁止改动范围。
...
6. 当代码、代码审查与测试预检收敛后，主 Agent 自己执行 `final-evidence-gate`；证据不足或高风险时再运行 `verify-ios-build`。
7. 如果最终证据门禁或升级验证失败，主 Agent 把首个真实失败点、影响范围和验证基线回写给 coder，再进入下一轮修复。
...
- **Step 5 主 Agent 聚合与裁决**
  - 回写规则、最多 2 轮回环、`final-evidence-gate` 裁决；必要时升级 `verify-ios-build`
...
- `main agent`
  - 负责聚合、回写、轮次控制、最终 `final-evidence-gate`，以及必要时执行 `verify-ios-build`。
...
- 最终 completion gate 始终由主 Agent 独占执行 `final-evidence-gate`；任何 subAgent 都不能替代最终证据裁决，也不能决定任务已完成。需要补构建证据时，`verify-ios-build` 也只能由主 Agent 执行。
- 如果同一任务中已经先跑过定向测试或其它 build/test，最终证据门禁默认复用同一套 workspace / scheme / destination 基线。
...
- `references/apple-gate-rules.md`：Apple/Xcode 项目的最终证据门禁特殊约束。
...
- 测试内部路由到 `testing`，最终收尾门禁由主 Agent 切 `final-evidence-gate`；必要时再切 `verify-ios-build`。
