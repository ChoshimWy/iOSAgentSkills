...
Run and explain GitNexus CLI commands for indexing, status checks, cleanup, wiki generation, and repo listing.
...
```json
{
  "goal": "index/status/clean/wiki/list intent, target repo root, optional flags such as --force, --pdg, or --embeddings.",
...
```json
{
  ...
  "command_plan": [],
  "known_risks": [],
  "next_action": "use-gitnexus-skill | ask-user | blocked"
}
```
## Exit Conditions
...
Use after `gitnexus-guide` detects a missing/stale index, or when the user explicitly asks to index or manage GitNexus data.
## Token Budget
