...
Use and maintain GitNexus `--pdg` CFG, reaching-definition, and taint-analysis layers for source-to-sink reasoning.
...
```json
{
  "goal": "taint finding/question, source/sink/sanitizer change, repo name, PDG/index mode.",
...
```json
{
  ...
  "summary": [],
  "taint_model_assessment": [],
  "known_risks": [],
  "next_action": "add-tests | pdg-query | read-source | blocked"
}
```
## Exit Conditions
...
Use for taint engine design and findings; use `gitnexus-pdg-query` for anchored CDG/REACHING_DEF reads.
## Token Budget
