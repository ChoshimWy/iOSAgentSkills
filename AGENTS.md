...
- 审查型任务默认交给独立 reviewer subAgent 执行；没有阻塞项时要明确说明 `阻塞问题：无`，并指出剩余风险或验证缺口。
...
- 新增 `.swift` / `.h` / `.m` / `.mm` 文件且项目要求文件头时，先参考同目录已有文件头；`Created by` 必须使用 `whoami` 或 `id -un` 的真实本机用户名称，不写 `Codex`、字面量 `$(whoami)` 或占位符；日期默认 `YYYY/M/D`；创建后重新打开新增源文件复查。
