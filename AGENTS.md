...
- 所有可见的进度说明、审查结论、最终回复、复核矩阵默认优先参照 Claude CLI 的可读终端风格：先给清晰结论，再用紧凑 Markdown 表格承载证据、风险与下一步；不要求也不展示隐藏推理过程。
- 表格适用于多文件、多风险、多步骤、多证据场景；1-2 条简单答复可用简短列表，避免为低信息量内容强行造表。
- 表格后必须给出一句明确结论；如果存在阻塞项，先标明 `blocked` / `pending review` / `env_issue`，再列证据。
- 常用列优先使用：`#`、`对象 / 文件`、`结论 / 改动要点`、`证据 / 验证`、`必要性 / 风险`、`状态 / 下一步`。
- 必要性与风险标记统一：`✅ 必要 / 通过`、`⚠️ 可选 / 有风险`、`❌ 不建议 / 阻塞`；保持终端友好，避免过宽表格，必要时拆成多个小表；不要用表格替代必要的代码块、命令或原始报错摘录。
